...
def main():
    # set <backbone>, <direct_inference>, <prompt>
    backbone = 'LLaMA2-7b'
    ...
    if direct_inference:
        prompt = 'Knowledge_Enhancement'
        assert prompt in ['PromptEOL', 'Pretended_CoT', 'Knowledge_Enhancement']

        if prompt == 'PromptEOL':
            manual_template = PromptEOL
        elif prompt == 'Pretended_CoT':
            manual_template = Pretended_CoT
        elif prompt == 'Knowledge_Enhancement':
            manual_template = Knowledge_Enhancement
        else:
        ...
        prompt = 'PromptSTH'
        lora_weight_path = './mistral-7b-lora-sth/checkpoint-1000'
        assert prompt in ['PromptEOL', 'PromptSTH', 'PromptSUM']

        if prompt == 'PromptEOL':
            manual_template = PromptEOL
        elif prompt == 'PromptSTH':
            manual_template = PromptSTH
        elif prompt == 'PromptSUM':
            manual_template = PromptSUM
        else:
            raise ValueError('Unknown prompt.')        
    ...
    def batcher(params, batch, max_length=None):
        ...
        with torch.no_grad():
            ...
            if args.avg:
                ...
                if direct_inference and prompt in ['Pretended_CoT', 'Knowledge_Enhancement']:
